/**
 * ===============================
 * CARRUSEL DE REDES SOCIALES V2 - SIMPLIFICADO
 * ===============================
 */

class SimpleSocialCarousel {
    constructor(containerId) {
        ...
        this.socialNetworks = [
            ...
            { 
                ...
                color: 'linear-gradient(135deg, #E4405F, #833AB4, #F77737)'
            },
            { 
                name: 'LinkedIn',
                ...
                color: 'linear-gradient(135deg, #0A66C2, #2196F3)'
            },
            { 
                name: 'X / Twitter',
            ...
            { 
                ...
                color: 'linear-gradient(135deg, #FF4500, #FF6314)'
            }
        ];
    ...
    init() {
        console.log('[SIMPLE CAROUSEL] Iniciando...');
        this.render();
        this.bindEvents();
        this.selectNetwork(0); // Auto-seleccionar Facebook
        console.log('[SIMPLE CAROUSEL] ✅ Listo!');
    }

    render() {
        // Crear HTML simplificado
        const html = `
            <div class="simple-social-carousel">
                <div class="carousel-header">
                    <h4>Selecciona tu red social:</h4>
                </div>
                
                <div class="current-selection">
                    <div class="current-icon" id="currentIcon">
                        ${this.socialNetworks[0].svg}
                    </div>
                    <div class="current-name" id="currentName">
                        ${this.socialNetworks[0].name}
                    </div>
                </div>
        ...
        // Referencias DOM
        this.currentIcon = this.container.querySelector('#currentIcon');
    ...
    updateSelection() {
        ...
        // Disparar evento
        const event = new CustomEvent('socialNetworkChanged', {
            detail: {
        ...
        console.log(`[SIMPLE CAROUSEL] Seleccionado: ${selectedNetwork.name}`);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('[SIMPLE CAROUSEL] DOM cargado, buscando contenedor...');
    
    const container = document.querySelector('.social-network-selector');
    if (container) {
        // Asegurar ID
        if (!container.id) {
            container.id = 'socialCarouselContainer';
        }
        
        // Inicializar
        window.socialCarousel = new SimpleSocialCarousel(container.id);
        console.log('[SIMPLE CAROUSEL] ✅ Inicializado correctamente');
        ...
        container.addEventListener('socialNetworkChanged', function(e) {
            ...
            if (note && note.textContent.includes('ideas diferentes')) {
                note.innerHTML = `🎯 Obtendrás 3 ideas diferentes para <strong>${e.detail.network}</strong>`;
            }
        });
    } else {
        console.error('[SIMPLE CAROUSEL] ❌ No se encontró el contenedor .social-network-selector');
    }
});
...
function getSelectedSocialNetwork() {
    ...
    return hiddenInput ? hiddenInput.value : 'Facebook';
}
